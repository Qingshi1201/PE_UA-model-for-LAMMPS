...
for mol_id in range(total_mols):
    ...
    if mol_id in selected_mols:
        ...
        # Assign every 34th interior monomer as type 2 (make functionality=4)
        for i in range(1, mol_length-1):
            if i % 34 == 0:
                bead_id = first_bead_id + i
                bead_types[bead_id] = 2
                type2_beads.append(bead_id)
    else:
...
def crosslink_by_density(
    ...
    for dist, bead1, bead2 in possible_bonds:
        ...
        if bead1 in bead_mol_connections and mol2_id in bead_mol_connections[bead1]:
            continue
        if bead2 in bead_mol_connections and mol1_id in bead_mol_connections[bead2]:
            continue
    ...
    print(f"Actual extent of reaction: {actual_reaction_extent:.3f}")
    print(f"crosslinking bonds: {len(crosslinks)} out of {total_functional_bonds} possible crosslinking bonds have been formed")
